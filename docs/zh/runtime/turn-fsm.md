...
FSM 把新消息写入 `SessionStore`，向 `RunStore` 追加 `RunEventRecord`，并通过 `tokio::spawn` 排队一个异步事件持久化任务。如果还需要更多 turn（且 policy 允许），循环回到 `CheckingPolicy`。否则 run 以 `Completed` 或 `Failed` 终止。
