...
`behest` 是一个 Rust 原生的 agent 运行时库。它提供与 provider 无关的合约：chat、streaming、tool calling、embeddings、持久化、队列、RAG、可观测性 —— 由一组小而可热插拔的组件组装而成，可在运行时组合、替换与重载。
...
- **Operational surface** — 事件发布、快照、session gate、压缩、重试策略。
...
- **[配置与横切关注点](../config/agent-config.md)** — 配置、错误、可观测性、队列。
